...
export function Pipeline() {
  ...
  const handleApproval = useCallback(async (approved: boolean) => {
    ...
    try {
      ...
      // Dismiss this specific token — panel stays hidden even if AWS briefly
      // returns the same token on the next refresh while it finishes processing
      setDismissedToken(pending.token)
      setApprovalResult(approved ? '✅ Approved.' : '❌ Rejected.')
      // Keep polling; once token disappears from state, dismissedToken auto-clears below
      loadState(selectedName!, true)
    } catch (err: unknown) {
  ...
  // Clear dismissedToken once AWS confirms the token is gone
  useEffect(() => {
    if (dismissedToken && pendingApproval?.token !== dismissedToken) {
      setDismissedToken(null)
      setApprovalResult(null)
    }
  }, [pendingApproval, dismissedToken])
          ...
          <>
            ...
            {pipelineState && (
              <>
                ...
                {pendingApproval && pendingApproval.token !== dismissedToken && (
